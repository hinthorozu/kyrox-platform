...
- **Status:** Proposed — OL-01 through OL-07 accepted and implementation/certification complete; OL08-A accepted and OL08-01 complete; OL08-B technical export contract partially accepted; OL08-C credential-disposition contract accepted; OL08-D through OL08-G, OL-09 and OL-10 unresolved
- **Date:** 2026-08-26
...
The OL-08 readiness audit verified an additional cross-repository gap: current Core organization delete directly sets the Core tombstone after SYSTEM authorization but has no product-offboarding-complete prerequisite, while FAIR CRM previously had no durable closure execution state. Therefore Core tombstone cannot safely serve as the closure start signal. OL08-A assigns durable closure execution/progress state to FAIR CRM while keeping Core as lifecycle authority and reserves Core tombstone as the final cross-repository lifecycle mutation. OL08-01 implements that non-destructive state machine. OL08-B additionally accepts the technical shape of a closure-export completeness contract without treating existing feature exports as a closure-complete package. OL08-C now accepts a separate credential-disposition lifecycle rather than reusing normal email-account soft delete: outbound disablement, external invalidation evidence, local reusable send-secret zeroization and receive-only webhook-signing-secret handling remain distinct obligations.

## Decision state

OL-01 through OL-07 are accepted and their implementation/certification workstreams are complete. OL08-A is accepted and OL08-01 is implementation/certification complete. OL08-B is partially accepted for the technical export contract and bounded non-destructive planner scope. OL08-C is accepted for the bounded credential-disposition contract and authorizes OL08-04A. OL08-D through OL08-G, OL-09 and OL-10 remain proposals/open choices, and OL08-B still has unresolved package-lifecycle / `not_required` policy authority.
...
### 10. Organization deletion is the final step of an offboarding workflow, not the first step — PARTIALLY ACCEPTED / OL08-A + OL08-B + OL08-C 2026-09-08
...
OL08-A authorized OL08-01, a non-destructive execution/state-machine slice. OL08-01 is implementation/certification complete through FAIR CRM PR #255 and Platform PR #36. It does not export closure data, revoke credentials, destroy product data/artifacts or invoke Core tombstone.
...
OL08-02 closure-quiescence certification and bounded OL08-03A manifest/completeness planning are now complete through FAIR CRM PRs #256 and #257. Persistent/downloadable package materialization, package retention/expiry, customer-facing handover, `not_required` success and `integrity_verified` as an irreversible gate remain blocked.

**Accepted OL08-C credential boundary:** credential disposition is a separate FAIR CRM-owned phase tied to organization + closure execution + credential/account identity. It separates outbound disablement, external/provider-side invalidation, local reusable send-secret zeroization and receive-only webhook-signing-secret handling.

OL08-C requires:

- outbound credential use disabled before credential destruction,
- deterministic provider invalidation whenever a supported exact-credential capability exists,
- supported-but-unidentifiable provider credentials to fail closed rather than be treated as not-applicable,
- generic/provider-specific SMTP remote invalidation to require explicit evidence rather than being inferred from local password removal,
- local reusable send secrets to become unrecoverable from FAIR CRM storage after required external invalidation evidence,
- MailerSend/API send tokens and webhook signing secrets to have separate lifecycle semantics,
- receive-only webhook verification to authorize no outbound handoff,
- ambiguous provider invalidation outcomes to be durably blocked/reconciled rather than blindly auto-retried,
- final webhook-signing-secret purge before full credential-disposition completion; if the drain criterion is time-based, the duration remains an OL-09 decision.

OL08-C authorizes bounded **OL08-04A credential disposition state/evidence foundation**. It does not authorize product-data/artifact destruction, persistent closure-package lifecycle, retention duration, backup policy, cleanup-complete/tombstone-ready state or Core tombstone.
...
closure approved
  ...
  -> credential disposition under accepted OL08-C policy (bounded OL08-04A first)
  -> final webhook signing-secret purge only after its accepted drain criterion
...
- any time-based webhook signing-secret drain/retention criterion required by OL08-C,
...
| OL-08 | Closure/export/retention/delete sequence | Core tombstone exists; OL08-01 durable closure orchestration, OL08-02 certification and OL08-03A planner are implemented | OL08-A accepts closure orchestration; OL08-B partially accepts versioned policy-conditioned export technical contract; OL08-C accepts bounded credential disposition; product-data/artifact/retention/backup phases remain gated | **PARTIALLY ACCEPTED — OL08-A + OL08-B TECHNICAL CONTRACT + OL08-C 2026-09-08** |
| OL-09 | Retention/grace durations | Not defined | Business/legal decision required, including any time-based webhook drain | **OPEN CHOICE** |
...
OL08-B partially accepts the technical export contract in [P0_2_OL_08_B_EXPORT_DECISION.md](../P0_2_OL_08_B_EXPORT_DECISION.md). OL08-02 closure-quiescence certification and bounded OL08-03A export manifest/completeness planner are complete through FAIR CRM PRs #256/#257 and Platform PR #39. Persistent/downloadable package lifecycle and `not_required` policy authority remain gated.

OL08-C is accepted in [P0_2_OL_08_C_PROVIDER_CREDENTIAL_DECISION.md](../P0_2_OL_08_C_PROVIDER_CREDENTIAL_DECISION.md). The next authorized engineering slice is **OL08-04A credential disposition state/evidence foundation**. It may implement bounded credential-only state/evidence, deterministic provider capability/targeting support, external invalidation reconciliation, local send-secret zeroization and receive-only signing-secret state. It must not invent a time-based drain duration or open product-data/artifact/retention/backup/tombstone phases. OL08-D through OL08-G remain gated; OL-09 and OL-10 remain open choices.
...
- canonical organization lifecycle/tombstone mutation; product-specific closure progress remains outside Core.
...
- certified OL08-02 closure-quiescence behavior,
- implemented bounded OL08-03A export manifest/completeness planning,
- bounded OL08-04A credential disposition state/evidence, provider capability/targeting and send-secret disposition under accepted OL08-C,
- later closure package/data/artifact/final retention behavior only after the corresponding remaining OL08/OL-09/OL-10 decisions are accepted.
...
- define OL08-C's accepted credential-disposition contract as outbound-disable-first, evidence-based external invalidation, local reusable send-secret zeroization and receive-only webhook-signing-secret handling with fail-closed ambiguous-result semantics.

The ADR remains Proposed overall until the unresolved OL08-B package/policy gates, OL08-D through OL08-G, OL-09 and OL-10 decisions required by the chosen commercial lifecycle are resolved.
...
- [P0.2 OL-08C Provider Credential Disposition Decision](../P0_2_OL_08_C_PROVIDER_CREDENTIAL_DECISION.md)
