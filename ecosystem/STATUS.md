...
**The approved P0.2 identity / SaaS onboarding workstream is DONE as of 2026-08-29.** [ADR-0006](decisions/0006-organization-lifecycle-and-onboarding.md) remains Proposed overall. OL-01 through OL-07 are implemented/certified. OL08-A narrowly accepts non-destructive closure execution/orchestration, and **OL08-01 is now implementation/certification DONE** across FAIR CRM and Platform. OL08-B closure export obligation/contract is the active decision gate; OL08-C through OL08-G, OL-09 retention/grace policy and OL-10 backup implications remain separately gated.
...
**OL08-A closure orchestration is ACCEPTED and OL08-01 is DONE as of 2026-09-07.** FAIR CRM PR #255 added the FAIR CRM-owned durable non-destructive closure execution state machine, SYSTEM-authorized start/status/retry, live Core `SUSPENDED` precondition, idempotent duplicate/race handling, explicit blocked/retry evidence, transactional append-only local audit evidence and canonical tenant-isolation registration. Final head `aa34cd3e5d00ec6f7d6b7adaad4afa950319830e` passed Development Standard Gate #707 / run `34155567277` and Prod-Path E2E #270 / run `34155567315`, then merged to FAIR CRM `main` as `e47d4ffced9f963fd06bc263996d2d5d95e7c5f2`. Platform PR #36 passed Platform Standards CI #104 / run `34156643910` on head `ea77abf48ee606ace0756303075aec518eb2c4aa` and merged as `f08f3a334c56c3c1be31694af56bf65330e44fe9`, completing cross-repository certification. OL08-01 exposes no export, credential revoke/purge, product-data/artifact destruction, cleanup-complete/tombstone-ready transition or Core tombstone call. The implementation tracker is [P0.2 OL-08 Organization Offboarding Implementation Tracker](P0_2_OL_08_IMPLEMENTATION.md). **OL08-B closure export obligation/contract is now the active OPEN decision gate.** OL08-C through OL08-G, OL-09 and OL-10 remain unresolved.
...
FAIR CRM now also owns the certified OL08-01 durable closure execution state. A Platform SuperAdmin/SYSTEM-authorized caller may start, inspect or retry a closure execution only while live Core lifecycle authority reports `SUSPENDED`. The product persists execution/event evidence with database-enforced single-open/idempotency constraints and explicit blocked/retry semantics. This slice is deliberately non-destructive and cannot certify cleanup complete or invoke Core tombstone. Closure export and all destructive closure/retention/delete semantics remain separately gated; OL08-B is the active decision point.
