...
**FAIR CRM backend handoff, public auth UI and login integration are complete through CRM-UI-02.** CRM-BE-01 extended the thin Core auth client through FAIR CRM PR #86. CRM-BE-02 added the product `/api/v1/auth/*` bridge routes through PR #87. CRM-UI-01 added public `/signup`, `/activate`, `/forgot-password` and `/reset-password` screens through PR #88. CRM-UI-02 added the login entry links for account creation and password recovery through PR #89 while preserving existing login/session behavior and the explicit activation-to-login flow. FAIR CRM still owns no password policy, password hashing, action-token persistence or credential mutation. **The next execution phase is CRM-UI-03 — authenticated security/password-change settings.**
...
The approved P0.2 backend integration, public auth UI and login integration are delivered. `CoreAuthClient` supports signup/activation/forgot/reset/change-password; FAIR CRM exposes the thin bridge routes and public signup/activation/password-recovery screens, and login now links directly to signup and password recovery. The active product step is authenticated account/security password change. The existing Super Admin manual user-management flow remains preserved. Credential authority and password/token logic remain entirely in Core.
