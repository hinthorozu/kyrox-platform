...
**The approved P0.2 identity / SaaS onboarding workstream is DONE as of 2026-08-29.** [ADR-0006](decisions/0006-organization-lifecycle-and-onboarding.md) remains Proposed overall. OL-01 through OL-07 are implemented/certified. OL08-A is accepted and OL08-01 is DONE. OL08-B partially accepts the technical closure-export contract; OL08-02 closure-quiescence certification and the bounded OL08-03A export manifest/completeness planner are implemented/certified. OL08-C provider credential disposition is ACCEPTED and bounded OL08-04A is now cross-repository certified through FAIR CRM PR #258 + Platform PR #42. OL08-C is not fully complete because supported-but-unidentifiable MailerSend credentials remain fail-closed and final webhook signing-secret purge still depends on an accepted drain criterion. **OL-09 retention/grace decision-readiness is now the active lifecycle policy step; no duration is accepted yet.** OL08-D through OL08-G and OL-10 backup implications remain separately gated. `not_required` export policy authority and persistent/downloadable package lifecycle also remain gated.
...
**OL08-C is ACCEPTED and OL08-04A is CERTIFIED DONE within its bounded foundation scope as of 2026-09-08.** Platform PR #41 accepted the credential-only disposition contract and authorized OL08-04A. FAIR CRM PR #258 final head `ea12e43f0ba0066842630208217d27962fdde53c` passed Development Standard Gate #719 / run `34194802163` and Prod-Path E2E #279 / run `34194802141` final attempt 2, then merged as `f5fb4ad18165848ae632b71496a5e5d1cb7a403b`. Platform PR #42 final head `7d3de63c8072b908d8d3c2c38a3a386af22e3921` passed Platform Standards CI #116 / run `34196046348` and merged as `144ee611c1f061d10ec36c4713b08262a1465bf1`. The runtime adds migration `0079_closure_credential_dispositions`, durable credential disposition/event evidence, SYSTEM-only start/list/get/retry/reconcile surfaces, live `SUSPENDED` mutation checks, outbound disablement, explicit SMTP external-evidence-before-local-zeroization ordering, fail-closed `supported_unidentifiable` MailerSend handling and receive-only webhook signing-secret state. It does not call MailerSend token delete/pause APIs, does not treat unverified target metadata as deterministic targeting, does not final-purge webhook signing secrets and does not open product-data/artifact/retention/backup/tombstone phases.

**OL-09 retention/grace is now in decision-readiness, not implementation.** The readiness record is [P0.2 OL-09 Retention / Grace Decision Readiness](P0_2_OL_09_RETENTION_GRACE_DECISION_READINESS.md). The first recommended decision slices are OL09-A closure grace/reversibility + clock origin and OL09-B webhook receive-only drain criterion. No hours/days, purge schedule or destructive transition is accepted by readiness alone.
...
FAIR CRM remains the active M4 product. Its P0.1 tenant-isolation certification, P0.2 identity bridge, OL-07 suspension/reactivation runtime, OL08-01 closure execution, OL08-02 quiescence certification, OL08-03A export manifest/completeness planner and bounded OL08-04A credential disposition foundation are complete within their accepted boundaries.
...
Within accepted OL08-C scope, OL08-04A may durably classify and reconcile credential obligations, disable outbound account eligibility and zeroize a local SMTP send password only after explicit external invalidation evidence. Current-model MailerSend API tokens remain fail-closed as `supported_unidentifiable`; operator evidence cannot override that class, unverified token metadata does not authorize provider deletion/local token purge, and required webhook signing secrets may remain only in explicit `receive_only_pending` state.

The immediate lifecycle step is **OL-09 retention/grace decision-readiness**. No runtime is authorized by that readiness. Persistent/downloadable closure packages, `not_required` policy authority, unsupported/unidentifiable provider credential completion, final webhook signing-secret purge without an accepted drain criterion, product-data/artifact destruction, retention/grace duration execution, backup reconciliation, cleanup-complete/tombstone-ready state and Core tombstone remain gated.
...
| M3 Platform Services | Completed / historical | [archive/milestones/M3_PLATFORM_SERVICES.md](../archive/milestones/M3_FOUNDATION.md) |
