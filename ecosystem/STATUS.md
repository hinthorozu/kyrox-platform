...
**FAIR CRM backend handoff is now complete through CRM-BE-02.** CRM-BE-01 extended the thin Core auth client through FAIR CRM PR #86. CRM-BE-02 added the product `/api/v1/auth/*` bridge routes for signup, activation, forgot/reset and authenticated password change through FAIR CRM PR #87; Development Standard Gate #280 and Prod-Path E2E #149 passed before merge. FAIR CRM still owns no password policy, password hashing, action-token persistence or credential mutation. **The next execution phase is CRM-UI-01 — public signup/activation/password-recovery UI.**
...
The approved P0.2 backend integration is delivered: `CoreAuthClient` now supports signup/activation/forgot/reset/change-password and FAIR CRM exposes the thin bridge routes over those Core APIs. The active product step is now the public auth UI (`/signup`, `/activate`, `/forgot-password`, `/reset-password`), followed by authenticated account/security password change. The existing Super Admin manual user-management flow remains preserved. Credential authority and password/token logic remain entirely in Core.
