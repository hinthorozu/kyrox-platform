...
The FAIR CRM portion of the approved P0.2 workstream is now active: first extend the thin `CoreAuthClient`, then expose the bridge routes for signup/activation/forgot/reset/change-password, then add public/security UI while preserving the existing Super Admin manual user-management flow. Credential authority and password/token logic remain entirely in Core.
