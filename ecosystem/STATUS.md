...
**FAIR CRM self-service onboarding/security UI is complete through CRM-UI-03.** CRM-BE-01 extended the thin Core auth client through FAIR CRM PR #86. CRM-BE-02 added the product `/api/v1/auth/*` bridge routes through PR #87. CRM-UI-01 added public `/signup`, `/activate`, `/forgot-password` and `/reset-password` screens through PR #88. CRM-UI-02 added login entry links through PR #89. CRM-UI-03 added authenticated `/settings/security` password change through PR #90: it uses the existing Bearer bridge, keeps password policy/current-credential validation in Core, and after successful change clears local session state and returns to login after Core/bridge credential invalidation. FAIR CRM still owns no password hashing, action-token persistence or credential mutation. **The next execution phase is CRM-UI-04 — Super Admin user-management compatibility certification.**
...
The approved P0.2 backend integration and self-service UI are delivered through CRM-UI-03. `CoreAuthClient` supports signup/activation/forgot/reset/change-password; FAIR CRM exposes the thin bridge routes, public onboarding/recovery screens, login entry links and authenticated security/password-change settings. The active product step is certification that existing Super Admin manual user-management behavior remains intact and no new privileged-assignment path was introduced. Credential authority and password/token logic remain entirely in Core.
